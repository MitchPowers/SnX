...
func update_key(prefix,unmoor=,quiet=){
  current_pos = read_dyn(prefix,as_string=1);
  file = rdfile(prefix+".key");
  
  pllr = where( strpart(file, :14)=="RESTRAIN-PLLR ");
  if(numberof(pllr)>0){
    for(i=1; i<=numberof(pllr); i++){
      kword = "bird";
      coli = i;
      colx = coly= "0.0";
      kk = 0.50;
      ...
      sread,file(pllr(i)),format="%s %d %s %s %g %g\n",kword,coli,colx,coly,kk,dr;

      newx = current_pos(1,coli);
      newy = current_pos(2,coli);
      while( strpart(newx,5:5)!="." ) newx = " "+newx;
      while( strpart(newy,5:5)!="." ) newy = " "+newy;
      kk = is_void(unmoor)?kk:-kk; //note, this toggles 'remoor'
      
      file(pllr(i)) = swrite(format="%s  %3d  %s  %s  % 4g  %4g","RESTRAIN-PLLR ",coli,newx,newy,kk,dr);
    }
  }
  
  pshr = where( strpart(file, :14)=="RESTRAIN-PSHR ");
  if(numberof(pshr)>0){
    for(i=1; i<=numberof(pshr); i++){
      kword = "bird";
      coli = moli = i;
      colx = coly= "0.0";
      kk = 0.50;
      ...
      sread,file(pshr(i)),format="%s %d %d %s %s %g %g\n",kword,coli,moli,colx,coly,kk,dr;
      
      newx = current_pos(1,coli);
      newy = current_pos(2,coli);
      while( strpart(newx,5:5)!="." ) newx = " "+newx;
      while( strpart(newy,5:5)!="." ) newy = " "+newy;
      kk = is_void(unmoor)?kk:-kk; //note, this toggles "unmoor"
      
      file(pshr(i)) = swrite(format="%s  %3d  %3d  %s  %s  % 4g  %4g","RESTRAIN-PSHR ",coli,moli,newx,newy,kk,dr);
    }
  }
    
  new_key = create(prefix+".key");
...
func recut_key(fnkey,fnout,ncol,new_coords=,unmoor=,quiet=){
  ...
  for(i=1; i<=ncol; i++){
    ...
    */
    ...
    file(pllr(i)) = swrite(format="%s  %3d  % .18g  % .18g  % 4g  %4g","RESTRAIN-PLLR ",coli,newx,newy,kk,dr);
  }
...
func read_dyn(fn,as_string=){
  read_frame,fn+".xyz"
  qq = rdfile(fn+".dyn");
  txt = "user text"
  sread,qq(2),format="%d %s",NATO,txt;
  for(i=1; i<=NATO; i++){
    sread,qq(6+i),format=" %e %e %e",COORDS(1,i),COORDS(2,i),COORDS(3,i);
  }

  dat = array("",[2,2,NCOL])

  pp = rdfile(fn+".key");
  pllr = where( strpart(pp, :14)=="RESTRAIN-PLLR ");
  pshr = where( strpart(pp, :14)=="RESTRAIN-PSHR ");
  pllri = indgen(numberof(pllr));
  pshri = indgen(numberof(pshr));
  for(i=1; i<=numberof(pllr); i++){
    kword = "bird";
    coli = i;
    colx = coly= 0.0;
    kk = 20.0;
    dr = 1.5;
    sread,pp(pllr(i)),format="%s %d %g %g %g %g\n",kword,coli,colx,coly,kk,dr;
    pllri(i) = coli;
    //write,format="PLLR on column %2d\n",coli;
  }
  for(i=1; i<=numberof(pshr); i++){
    kword = "bird";
    coli = moli = i;
    ...
    kk = 20.0;
    dr = 1.5;
    sread,pp(pshr(i)),format="%s %d %d %g %g %g %g\n",kword,coli,moli,colx,coly,kk,dr;
    pshri(i) = coli;
    //write,format="PSHR on column %2d with mol %2d\n",coli,moli;
  }  

  //now check the dyn file for PSHR/PLLR info
  pllr = where( strpart(qq, :5)==" PLLR");
  if(numberof(pllr)>0) pllr = pllr(1);
  pshr = where( strpart(qq, :5)==" PSHR");
  if(numberof(pshr)>0) pshr = pshr(1);
  switchFlag = 0;
  if(numberof(pshr)==0){
    //no push, only pull
    //write,format="Don't see a pshr in dyn. Using column %2d, from key",pshri(1);
    switchFlag = pshri(1);
    grow,pllri,999;
  }
  ...
  dat = array("",[2,2,NCOL]);
  if(numberof(pshr)>0){
    for( i=1; i<=numberof(pshri); i++){
      sread,qq(pshr+i),format="%s %s",dat(1,pshri(i)),dat(2,pshri(i));
    }
  }
  if(numberof(pllr)>0){
    for(i=1; i<=numberof(pllri); i++){
      if(switchFlag==i){
        sread,qq(pllr+i),format="%s %s",dat(1,pshri(1)),dat(2,pshri(1));
        continue;
      }
      j = i - (i>switchFlag);
      sread,qq(pllr+i),format="%s %s",dat(1,pllri(j)),dat(2,pllri(j));
    }
  }
  if(as_string==1) return dat;
  return double(tonum(dat));
}
  

func hex_em(prefix,fnout,qarr,num,rand=,lil=){
  /* DOCUMENT builds a 19 column system
     hex_em(prefix, fnout, qarr, num, rand=)
    qarr is a double array of the molecular spacing in each direction
         specifically,  [d,c]
    num is molecules per column
  */
  ncol = is_void(lil)?13:lil;
  read_frame,prefix+".xyz";
  file=create(fnout+".xyz");
  write,file,format=" %d Generated from %s",NATO*ncol*num,prefix;
  write,file,format="\n 120  120  %g  90.000 90.000 90.000",num*qarr(2);
  offset=[0.,0.,0.];
  coords=COORDS;
  for(i=0; i<=6; i++){
    for(j=1; j<=num; j++){
    ...
    if(ncol>7){
    if(i>0){
      offset=qarr(1)*[cos(i*pi/3 + pi/6),sin(i*pi/3 + pi/6),0.];
      for(j=1; j<=num; j++){
        coords=COORDS+offset;
        th=2.*pi*random();
        if(rand==0) th=rand;
        ...
        coords+=cm+offset*[sqrt(3),sqrt(3),1];
    ...
    }//move offset and make next column
  ...
  read_frame,fnout+".xyz";
  COORDS -= COORDS(,avg);
  write_xyz,fnout+".xyz";
  plot_n_rot,atom_plot;

  keytxt = open(prefix+".key");
  keytxt = rdfile(keytxt);
  outkey = create(fnout+".key");
  for(n=1; n<=numberof(keytxt); n++){
    write,outkey,format="%s\n",keytxt(n);
  }
  write,outkey,format="%s","\n";
  write,ncol;
  collat = split(aa_cm(),ncol)(:2,avg,);
  for(n=1; n<=ncol; n++){
    k = 20.0;
    dr= 1.5;
    if(n==1){
      //write,outkey,format="RESTRAIN-PSHR %d %d 0.00 0.00 0.500 -.500 \n",n,11;
      write,outkey,format="RESTRAIN-PSHR   %3d  %3d  0.00  0.00   0.500   0.00 \n",n,11;
    }else{
    write,outkey,format="RESTRAIN-PLLR   %3d  % .18g  % .18g   0.500   0.0 \n",n,collat(1,n),collat(2,n);
    }
    plot,collat(1,n),collat(2,n);
  }
 ...
 ********************************************************
 *******************************************************/
func plot_n_rot(plotfunc){
  if( !is_func(plotfunc) ) return "Huh?";
  og = COORDS;
  COORDS-= COORDS(,avg);
  plotfunc;
  plotl,DIMS(1)*[-1,1,1,-1,-1]*.5,DIMS(2)*[1,1,-1,-1,1]*.5,color="black",width=3;
  glob_spin,pi/2,raxis=1;
  offset = (DIMS(2)+DIMS(3))*.625;
  COORDS(2,)+= offset;
  plotfunc;
  plotl,DIMS(1)*[-1,-1,1,1]*.5,offset + DIMS(3)*[1,-1,-1,1]*.5,color="black",width=3;
  COORDS(2,)-=(DIMS(2)+DIMS(3))*.625;
  glob_spin,-pi/2,raxis=1;
  
  glob_spin,pi/2,raxis=2;
  offset = (DIMS(1)+DIMS(3))*.625;
  COORDS(1,)+= offset;
  plotfunc;
  plotl,offset + DIMS(3)*[1,-1,-1,1]*.5,DIMS(2)*[1,1,-1,-1]*.5,color="black",width=3;

  COORDS(1,)-=(DIMS(1)+DIMS(3))*.625;
  glob_spin,-pi/2,raxis=2;

  plotl,(DIMS(1)+DIMS(3))*.625*([-1,1,1,-1,-1]+.625),(DIMS(2)+DIMS(3))*.625*([1,1,-1,-1,1] + .625),color="grayd",width=1;

  limits,e,e,e,e;
  square_limits;
}

func proj_plot(void,nvec=,width=,color=,res=,molecs=,cols=){
  if(is_void(nvec)) nvec = [0.,0.,1];
  res = is_void(res)?36:res;
  molecs = is_void(molecs)?indgen(NCOL*MPC):molecs;

  coords = perp2(COORDS,nvec);

  temp = array(0.,[2,2,res+1]);
  rcut = 1.4;
  if(cols){
    mpc = MPC;
    nmol = NMOL;
  ...
  for(m=1; m<=NCOL*MPC; m++){
    if(noneof(molecs==m)) continue;
    alph = (m-1)*NMOL+1;
    omeg = m*NMOL;
    mcoord = coords(,alph:omeg);
    mcentr = mcoord(,avg)
    mcoord-= mcentr;
    adists = norm(mcoord);
    afar = where(adists==max(adists))(1);
    aphi = atan(mcoord(2,afar),mcoord(1,afar));
    for(n=0; n<=res; n++){
      phi = aphi + n*2*pi/res;
      vec = [cos(phi),sin(phi),0.];
      vpara = para2(mcoord,vec);
      vperp = mcoord - vpara;
      dperp = norm(vperp);
      if(noneof( (dperp < rcut)*(vpara(+,)*vec(+)>0) ) ) continue;
      dpara = norm(vpara);

      ww = where( (dperp < rcut)*(vpara(+,)*vec(+)>0) );

      iang = acos(dperp(ww)/rcut);
      ipara = dpara(ww) + dperp(ww)*tan(iang);
      
      mm = ww( where(ipara==max(ipara) ) )(1);

      tvec = vpara(,mm) * (max(ipara)/dpara(mm));

      temp(,n) = tvec(:2,);
    }
    temp(,0) = temp(,1);
    temp+= mcentr(:2,-);
    plotl,temp(1,),temp(2,),width=width,color=color;
...
func atom_plot(void,width=,colors=){
  atoms=swatch=NMS;
  ...
  for(i=1; numberof(swatch)>=1; i++){
    ...
    for(j=1; j<=numberof(inds); j++){
      ll=span(0.,2*pi,12);
      cx=[COORDS(1,inds(j))+rarr(i)*cos(ll),COORDS(2,inds(j))+rarr(i)*sin(ll),COORDS(2,inds(j))];
      plotl,cx(,1),cx(,2),color=colors(i),width=width;
    }
...
func parse_traj(prefix,start=,stop=,dir=,prefunct=,funct=,postfunct=,color=,verbose=){
  ...
  */
  ...
  if(numberof(dir)>1){
    ncfs = array(0,numberof(dir)+1);
    for(d=1; d<=numberof(dir); d++){
      ncfs(d+1) = count_files(prefix,dir=dir(d));
    }

    NCF = ncfs(sum);
    longcat = array(0.,[3,10,NATO/NMOL,NCF]);
    longdim = array(0.,[2,3,NCF]);
    longdat = array(0.,[1,NCF]);
    
    for(d=1; d<=numberof(dir); d++){
      if(ncfs(d+1)==0){
        write,format="Warning: no snapshots in %s",dir(d);
        continue;
      }
      frame_out = parse_traj(prefix,dir=dir(d));
      alph = ncfs(:d)(sum) + 1;
      omeg = ncfs(:d+1)(sum);
      write,alph,omeg;
      longcat(:,:,alph:omeg) = CAT;
      longdim(:,alph:omeg) = DIM_ARR;
      longdat(alph:omeg) = frame_out;
    }
    CAT = longcat;
    DIM_ARR = longdim;
    return longdat;
  ...
  if(verbose) write,format="Stop parse at: %d\n",stop;
  if(is_void(start)) start = 1;
  if(start < 0) start = max(stop - ogcf + 1, stop + start);
  if(start > stop){
    write,format=" Error: %s\n","Bad start/stop";
    return -999;
  }
  offset = start - 1;
  NCF = stop - offset; //Total number of files to be read
  if(verbose) write,format="Reading from %d to %d (total %d)\n",start,stop,NCF;
  ...
  for( i=1+offset; i<=stop; i++){
    ...
    if(is_func(funct)) frame_dat(i-offset) = funct();
  ...
  if(verbose) write,format="Externals:\n %s\n","DIM_ARR","CAT";
...
func voroneigh(cms,dims,tag=){
  //assumes there's an external cat
  numm = dimsof(cms)(0);
  inds = indgen(numm);
  vneigh = array(0.,[2,32,numm]);
  vweigh = array(0.,[2,32,numm]);
  mneigh = array(0,32);

  hdim = totxt(dims*.5);

  tag = is_void(tag)?totxt(int(random()*1000000)+1):tag;

  tmpf = create("tmp.voro"+tag);
  write,tmpf,format="%d %g %g %g\n",inds,cms(1,),cms(2,),cms(3,);
  close,tmpf;
  cstr = "voro++ -g -pz -c \"%i     %n %f\"  -";
...
func read_charges(key){
  qq=rdfile(key);
  cx = where(strpart(qq,:7)=="CHARGE ");
  chg=array(0.,30);
  atm=indgen(30);
  sread,strpart(qq(cx),8:),format="%d %g",atm,chg;
        //chg = tonum(strpart(qq(cc),11:));
  extern CHG;
  CHG = chg;
  write,format="Net Charge: %.5f e\n",sum(chg);
  return chg;
}
...
func keyvor(molec){
  vv = voroneigh(aa_cm(),DIMS);
  vv = int(vv(,,1));
  v1 = vv(,molec);
  v1 = v1(where(v1));
  v2 = v1;
  return v1;
  for(n=1; n<=numberof(v1); n++){
    ne = vv(,v1(n));
    for(i=1; i<=numberof(ne); i++){
      if( noneof(ne(i)==v2) ){
        grow,v2,ne(i);
      }
    }
...
func cat2cols(cat,eqt=,tavg=){
  eqt = is_void(eqt)?1:eqt;
  dat = cat(:2,,eqt:);
  mcat = reform(dat,[4,2,MPC,NCOL,dimsof(dat)(0)]);
  colxy = mcat(,avg,,);
  if(tavg) colxy = colxy(,,avg);
  return colxy;
}


func coord2col(void){
  mcat = reform(aa_cm()(:2,),[3,2,MPC,NCOL]);
  colxy = mcat(,avg,);
  colxy = (pbc(mcat - colxy(,-,)) + colxy(,-,))(,avg,);
  return colxy;
}

func col2col(colcoords){
  c2c = norm( pbc(colcoords(:2,-,..) - colcoords(:2,..,-)) );
  c2c(where(c2c==0)) = 999.;
  neigh = where2( c2c == c2c(min,)(-,))(1,);
  return neigh;
}

func col2mol(colcoords){
  c2m = norm( pbc(aa_cm()(:2,-,..) - colcoords(:2,..,-)) );
  neigh1 = where2( c2m == c2m(min,)(-,))(1,);
  c2m(where( c2m==c2m(min,)(-,) )) = 999.;
  neigh2 = where2( c2m == c2m(min,)(-,))(1,);
  return transpose([neigh1,neigh2]);
}

func frame_rx(colcoords){
  cij = col2mol(colcoords);
  a = norm( pbc(colcoords(:,cij(1,)) - aa_cm()(:2,) ));
  c = norm( pbc(colcoords(:,cij(1,)) - colcoords(:,cij(2,)) ));
  return a/c;
}
